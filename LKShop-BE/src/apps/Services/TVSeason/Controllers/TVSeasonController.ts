...
export class TVSeasonController {
    ...
    public async GetListTVSeason(req: Request, res: Response) {
        const filter = req.query as unknown  as TVSeasonFilter
        const pageable = req.query as unknown as Pageable
        const tvseasons = await getListTVSeasonHandler(filter,pageable);
    ...
    private async UpdateTVSeason(req: Request, res: Response, next: NextFunction) {
        const { TVSeasonId } = req.params
        const Movie = await updateTVSeasonHandler(TVSeasonId, req.body, req.files)
