...
} from "../../SingleMovie/Repositories/SingleMovieRepository";
...
export class SiteController {
    ...
    const create_payment_json = {
      ...
      redirect_urls: {
        return_url:
          ...
          "/" +
          baseUrl +
          `/PaySuccess?Price=${body.Price}&ClientId=${body.ClientId}&Email=${body.Email}&Bundle=${body.BundleName}`,
        cancel_url: "/cancel",
      },
      transactions: [
        {
  ...
  private async getMovieByName(
    ...
    try {
      ...
      return res
        .status(200)
        .send(new BaseResponse<Movie[]>(movies, "Get Success", true));
    } catch (error) {
      console.log(error);
    }
  }
  ...
  private async getMovieByCategory(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
