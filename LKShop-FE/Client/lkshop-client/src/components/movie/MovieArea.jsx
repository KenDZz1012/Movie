...
function SearchArea({ movies, size, totalPage, page, category, getListMovie, catId }) {
        ...
        <div className="row align-items-end mb-60">
          <div className="col-lg-6">
            <div className="section-title text-center text-lg-left">
              <span className="sub-title">Category: {category}</span>
            </div>
          </div>
          <div className="col-lg-6">
            ...
            <div className="pagination-wrap mt-30">
              <nav>
                <ul>{[...Array(totalPage)].map((e, i) => {
                  return <li className={page == i + 1 ? `active` : "  "}><Link to={`/movie-Category=${category}-Page=${i + 1}`} onClick={() => { getListMovie(catId, i + 1) }}>{i + 1}</Link></li>
                })}
                </ul>
              </nav>
            </div>
